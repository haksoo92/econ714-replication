...
from customenv import rlfund as rlfund
...
infoLsDisp = ["Open", "High", "Low", "Close"]
...
eval_py_env = rlfund.PriceInfoEnv(df_all, infoLs)
...
for i in range(n_factors):
    ...
    if i==n_factors-1:
        plt.setp(axs[2+i], xlabel='Period in Episode')
    plt.setp(axs[2+i], ylabel=infoLsDisp[i])
    axs[2+i].legend(loc='lower right')

fig.set_size_inches(12, 6 + 3*n_factors + 3)
plt.ylim()
# plt.show()
# plt.close()
plt.savefig(epi_fig)
...
fig.suptitle("Performance of the RL Agent in Training")
steps = (np.array([*range(0, len(returns))]) + 1) * eval_interval
# axs.fill_between(steps, returns[:,0] - returns[:,1], returns[:,0] + returns[:,1], alpha=0.25)
axs.plot(steps, returns, 'k')
plt.setp(axs, xlabel='Training Epoch')
