...
class PriceInfoEnv(py_environment.PyEnvironment):
    ...
    def __init__(self, df_all, infoLs):
        """
        Scripts
        ...
        if 'prccd' in infoLs:
            xy_m3mo, xy_p12mo = cutil.getEpisodeData(df_all, infoLs)
        else:
            xy_m3mo, xy_p12mo = cutil.getEpisodeData(df_all, infoLs + ["prccd"])
    ...
    def _reset(self):
        if 'prccd' in self._infoLs:
            xy_m3mo, xy_p12mo = cutil.getEpisodeData(self._df_all, self._infoLs)
        else:
            xy_m3mo, xy_p12mo = cutil.getEpisodeData(self._df_all, self._infoLs + ["prccd"])
